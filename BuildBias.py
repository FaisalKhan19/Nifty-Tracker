...
output_file = "announcement_analysis.csv"
...
for resFile in tqdm(os.listdir(base_dir), desc='Processing Quarter'):
    ...
    for _, ann in tqdm(results_df.iterrows(), desc='Processing Announcements', total=len(results_df)):
        ...
        if trading_start <= annTime <= trading_end:
            ...
            bias_change = df_post['close'].iloc[-1] - df_post['close'].iloc[0]
            bias = "Bullish" if bias_change > 0 else "Bearish"
